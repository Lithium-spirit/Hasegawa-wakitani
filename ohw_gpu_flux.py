...
def compute_particle_flux(n, phi, KY):
    """Calculates Gamma = <n * v_x> where v_x = -d(phi)/dy"""
...
print("Starting GPU simulation to track Particle Flux (OHW)...")
...
for step in range(steps):
    ...
    if step % record_every == 0:
        ...
        print(f"Step {step}/{steps} | Time: {current_time:.1f} | "
              f"Flux: {current_flux:.4f} | "
              f"Elapsed: {elapsed:.1f}s | ETA: {remaining/60:.1f} min", end='\r')
...
plot_filename = "flux_vs_time_ohw.png"
