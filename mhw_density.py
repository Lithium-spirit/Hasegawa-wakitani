...
def compute_derivs_mhw(omega, n, phi, K_sq, KX, KY, alpha, Dn, nu, kappa):
    
    phi_tilde = phi - zonal_mean(phi)   # strip zonal mean from phi
    n_tilde   = n   - zonal_mean(n)     # strip zonal mean from n
...
def rk4_step(omega, n, phi, dt, K_sq, KX, KY, alpha, Dn, nu, kappa):
    def derivs(w, nn):
        p = get_phi(w, K_sq)
        return compute_derivs_mhw(w, nn, p, K_sq, KX, KY, alpha, Dn, nu, kappa)
...
print("Starting GPU simulation — Zonal Energy & Density Fluctuations (mHW)...")
...
plt.tight_layout()
plot_filename = "zonal_density_vs_time_mhwn.png"
